...
const typeDefs = /* GraphQL */ `
  ...
  type Query {
    schema(tableSearch: String, columnSearch: String): [Table!]!
  }
`;
...
const resolvers = {
  Query: {
    schema: async (
      ...
      // Return type is an array of Table objects
      ...
      try {
        ...
        // Initialize array to hold table schemas
        const schema: Table[] = [];

        // Step 2: Loop through each table and fetch columns, keys, etc.
        for (const table of tablesResult.rows) {
          ...
          // Step 2b: Apply column search if provided
          if (columnSearch) {
            columnsQuery += ` AND (column_name ILIKE $2 OR data_type ILIKE $2`;

            // Special handling if user searches for "primary"
            ...
            // Special handling if user searches for "foreign"
            ...
            columnsQuery += `)`;
            // Add parameter for ILIKE
            columnParams.push(`%${columnSearch}%`);
          }

          // Order columns as defined in table
          ...
          // Skip tables with no matching columns if filtering by columnSearch
          if (columnSearch && columnsResult.rows.length === 0) continue;

          // Step 2c: Fetch primary keys for current table
          ...
          // Step 2d: Fetch foreign keys for current table
          ...
          // Step 2e: Construct Column objects with primary key info
          const columns: Column[] = columnsResult.rows.map((col) => ({
            ...col,
            is_primary_key: primaryKeys.includes(col.column_name),
          }));

          // Step 2f: Construct Table object and add to schema array
          schema.push({
            ...table,
            comment: null, // No comments fetched in this version
            columns, // All columns
            primary_keys: primaryKeys, // List of primary key column names
            foreign_keys: foreignKeyResult.rows, // List of foreign key objects
          });
        }

        return schema; // Return full schema to GraphQL client
      } finally {
        client.release(); // Release PostgreSQL client back to pool
      }
    },
  },
};
