...
import { createLightweightSqlCompletion } from "@/app/utils/sqlCompletion/lightweightCompletion";
...
export default function CodeMirrorEditor({
  ...
  const sqlCompletion = useMemo(
    () => createLightweightSqlCompletion(tableNames, tableColumns),
  ...
  useEffect(() => {
    // Don't update during tab switches
  ...
  }, [selectedTable, selectedColumns, activeTab, updateTabState]);
  ...
  useEffect(() => {
    ...
    if (shouldUpdate) {
      // Mark this as a programmatic change to prevent the updateListener from calling onQueryChange
      isProgrammaticChangeRef.current = true;
      ...
      // Use setTimeout to ensure it resets after the change event fires
      setTimeout(() => {
        isProgrammaticChangeRef.current = false;
      }, 0);
    }
    ...
    () =>
      EditorView.updateListener.of((update) => {
        if (update.docChanged) {
          ...
          // Update the tab query
          queryCallbacksRef.current.updateTabQuery(
            queryCallbacksRef.current.activeTab,
            newQuery
          );

          // Skip calling onQueryChange if this was a programmatic update from parent
          // This prevents the infinite loop where parent updates cause editor updates which cause parent updates
